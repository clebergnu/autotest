"""
This module provides a series of utilities to communicate with Koji.

Koji is a a build server used by the Fedora project.

These modules allow test code to inspect builds available on the Koji server,
as well as download the packages generated by those builds.
"""
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == 'a':
            for key, value in attrs:
                if key == 'href' and value.endswith('.rpm'):
                    self.package_file_names.append(value)
        ...
        if pkg.subpackages:
            for subpackage in pkg.subpackages:
                for pfn in index_parser.package_file_names:
                    rpm = RPMFileNameInfo(pfn)
                    info = rpm.get_nvr_info()
                    if (subpackage == info['name'] and
                        rpm.get_arch() in arches):
...
def set_default_koji_tag(tag):
    '''
    Sets the default tag that will be used by koji
    '''
...
def get_default_koji_tag():
    '''
    Returns the default tag that will be used by koji
    '''
...
class KojiPkgSpec(object):
    ...
    part of build number 1000:
        ...
        >>> from autotest.client.shared.koji_utils import KojiPkgSpec
    ...
    # pylint: disable=I0011,R0913,W0102
...
class KojiScratchPkgSpec(object):
    ...
    from a scratch build (whose task id is 1000) sent by user jdoe:
        ...
        >>> from autotest.client.shared.koji_utils import KojiScratchPkgSpec
